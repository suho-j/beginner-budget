...
- 설정 탭에서 선택한 예산기간의 남은 항목 예산을 다른 항목으로 옮길 수 있습니다. 기본 흐름은 `비상금 → 생활비`이며 총예산은 바뀌지 않습니다.
...
예산과 반복지출 템플릿은 같은 settings 행에 저장합니다. 항목 간 예산 이동은 별도 거래나 이력을 만들지 않고 선택 기간의 `categoryBudgets` 두 값만 같은 금액만큼 증감합니다. 출발 항목의 설정 예산에서 해당 기간 실제 지출을 뺀 금액까지만 허용하며, 다른 예산 설정과 동일하게 DB의 `updated_at` 버전을 비교하는 원격 우선 CAS가 성공한 뒤 로컬 상태를 확정합니다. 템플릿 등록·수정·삭제도 같은 CAS를 사용합니다. JSON 가져오기·전체 초기화·샘플 교체는 설정 버전과 전체 거래 스냅샷을 함께 비교하는 5인자 RPC로 원자적으로 처리합니다.
...
2026-08-17 항목 간 예산 이동 작업 트리에서는 도메인 검증, 접근 가능한 폼, 360px 반응형 계약, Supabase 성공 전 로컬 미반영과 실패 롤백을 포함해 `96 tests passed`를 확인했습니다. 운영 배포 결과는 GitHub Pages `master`의 source commit과 공개 URL에서 별도로 확인하며, 실제 로그인 Supabase 브라우저 저장은 별도 검증 게이트입니다.
...
- [항목 간 예산 이동 구현·검증 계획](superpowers/plans/2026-08-17-category-budget-transfer.md)
