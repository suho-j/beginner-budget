...
1. `budget_settings`와 `transactions`를 백업하고 행 수를 기록합니다.
2. 기존 거래 ID가 `^[A-Za-z0-9._:-]+$` 규칙을 만족하는지 감사하고, 보정 전후 ID와 행 수를 대조합니다.
3. SQL을 적용한 뒤 ID 제약 조건, `updated_at` 트리거, 새 RPC 시그니처와 권한, 오래된 RPC 제거를 확인합니다.
4. 같은 스냅샷의 정상 저장과 오래된 스냅샷의 충돌 거부를 별도 테스트 계정 또는 격리 환경에서 검증합니다.
5. 백업·감사·검증 기록이 모두 확인된 뒤에만 공유 운영 데이터 미리보기를 허용합니다.
...
공유 운영 데이터로 검증하는 동안에는 **기존 운영 사이트의 모든 열린 탭을 닫고 운영 URL에서 저장하지 않아야 합니다.** 기존 운영 작성기는 전체 상태를 비원자적으로 저장하므로 새 미리보기 변경을 오래된 화면 상태로 덮어쓸 수 있습니다. 이 조건을 지킬 수 없다면 공유 운영 데이터 미리보기를 진행하지 않습니다.

운영 데이터를 쓰는 브라우저 스모크에서는 고유 메모 `QA-V1-<timestamp>`의 거래만 만들고 수정·삭제합니다. 원래 예산은 먼저 기록해 정확히 복원하며, JSON 가져오기와 전체 초기화는 실행하지 않습니다.
...
2026-08-12 로컬 자동 검증은 문법 검사와 `60 tests passed`를 통과했습니다. Supabase SQL 적용과 실제 로그인 브라우저 스모크는 아직 남아 있으므로 공개 미리보기 준비 완료로 간주하지 않습니다.
