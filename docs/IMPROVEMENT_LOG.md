...
- 공식 지출 카테고리에 `휴가비`를 추가해 거래 입력·수정, 필터, 항목별 예산, 예산 이동, 반복지출에서 동일한 검증 규칙으로 사용한다.
- `휴가비` 거래가 상태 정규화 과정에서 유지되는 회귀 테스트를 추가했다.
- JavaScript 문법 검사와 전체 자동 테스트 `97 tests passed`를 확인했다.

## 2026-08-17 · 항목 간 예산 이동

### 사용자 기능

- 설정 탭에 `어디에서 → 어디로 → 얼마를` 순서의 예산 이동 폼을 추가하고 기본값을 `비상금 → 생활비`로 두었다.
- 총예산과 거래를 바꾸지 않고 선택 예산기간의 두 항목 예산만 재배분한다.
- 출발 항목의 설정 예산에서 해당 기간 실제 지출을 뺀 금액까지만 허용하며, 같은 항목·미설정 출발 예산·잘못된 금액·DB 정수 상한 초과를 막는다.
- 명시적 라벨, live region, 첫 오류 포커스, 360px 한 열과 560px 이상 세 열 배치를 적용했다.

### 저장 안전성과 검증

- 새 DB 객체나 거래 이력을 만들지 않고 기존 settings `category_budgets.__monthly_budgets` 매핑을 사용한다.
- 기존 `BudgetCloud.saveSettings`의 `updated_at` CAS를 사용해 원격 저장 성공 뒤에만 로컬 상태와 화면을 바꾼다. 실패 시 예산과 입력 금액을 유지한다.
- TDD에서 정상 이동, 기간 내 지출 차감, 안전 경계, 마크업, UI 안내, 반응형 스타일, 저장 성공 전 로컬 미반영과 실패 롤백의 RED를 각각 확인했다.
- 2026-08-17 작업 트리에서 `node tests/run-tests.cjs`가 `96 tests passed`로 끝났다.
- 로컬 브라우저에서 설정 카드와 `비상금 → 생활비` 기본값, 360px 1열·1024px 3열, 가로 넘침 없음, 콘솔 error/warn 0건을 확인했다. 로그인 자격증명이 없는 로컬 미리보기였으므로 실제 Supabase 저장과 새로고침 후 유지 검증은 별도 게이트다.
- 이 항목은 운영 배포 전 검증 기록이다. 실제 배포 완료 여부는 `master` source commit, Pages workflow, 공개 URL 기능 표식을 함께 확인해 판정한다.
