...
- 거래 ID를 안전한 ASCII 형식으로 제한하고 DB 적용용 보정·제약 SQL을 추가했다.
...
2026-08-12, 소스 `d74f3f2`에서 다음 결과를 확인했다.
...
- DB 백업, 기존 ID 감사, ID 제약 조건, 단조 증가 트리거, RPC 시그니처·권한·충돌 동작 검증이 남아 있다.
- 실제 로그인 상태의 데스크톱·360×800·키보드·수정 창·콘솔 검증이 남아 있다.
- 운영 데이터에서 `QA-V1-<timestamp>` 거래 추가·수정·필터·캘린더·삭제, QA 0건 확인, 예산 원복 검증이 남아 있다.
- 공개 `/beginner-budget-preview/v1/` 배포, 소스 SHA 매니페스트 확인, 공개 URL 재검증이 남아 있다.

따라서 현재 결과는 **코드와 자동 테스트 완료**이며, DB 마이그레이션이나 공개 미리보기 검증 완료를 뜻하지 않는다.

### 공유 운영 데이터 하드 게이트

기존 운영 작성기는 오래된 전체 상태를 비원자적으로 저장할 수 있다. 공유 운영 데이터 미리보기를 하는 동안에는 모든 기존 운영 탭을 닫고 운영 URL의 쓰기를 중단해야 한다. SQL 선행 게이트 또는 이 사용 중단 조건을 만족하지 못하면 검증을 멈추고 격리 Supabase를 사용한다.
