...
- 가입/빌드 없는 정적 앱을 유지하되, 데이터 저장은 Supabase Auth + RLS 기반 클라우드 저장을 사용한다.
- 브라우저 `localStorage`에는 가계부 데이터를 저장하지 않는다.
...
- 사용자별 Supabase 데이터를 잘못 덮어쓰지 않는다.
